  ...
  if (questions.length === 0) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center space-y-4">
        ...
        <h1 className="text-xl font-bold text-ink">북마크가 없습니다</h1>
        <p className="text-ink-muted text-sm">문제를 풀면서 ☆ 버튼을 눌러 북마크를 추가해보세요.</p>
        <Link href="/quiz" className="inline-block px-5 py-2.5 bg-primary-600 text-white rounded-xl font-semibold text-sm hover:bg-primary-700 transition-colors">
          문제 풀러 가기
        </Link>
      </div>
    )
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-8">
        <div className="q-card text-center space-y-4">
          <div className="text-5xl">⭐</div>
          <h1 className="text-xl font-bold text-ink">북마크 문제 완료!</h1>
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6">
      <div className="flex items-center gap-2 text-sm text-ink-muted mb-4">
        ...
        <span>›</span>
        <span className="text-ink font-medium">북마크</span>
        <span className="ml-auto text-xs bg-yellow-50 text-yellow-700 border border-yellow-200 px-2 py-0.5 rounded-full font-semibold">
          ★ {questions.length}문항
        </span>
      </div>
